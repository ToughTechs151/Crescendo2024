 ...
 * <p>y = x
 */
public class LimitedDifferentialDriveSim {
  ...
  public void update(double dtSeconds) {
    ...
    // Calculate the tentative new system state
    m_x = NumericalIntegration.rkdp(this::getDynamics, m_x, m_u, dtSeconds);

    // If new position would exceed the field boundaries, use last position and stop wheel movement.
    Pose2d newPose = getStatePose();
    if ((newPose.getX() < m_xMin)
        || (newPose.getX() > m_xMax)
        || (newPose.getY() < m_yMin)
        || (newPose.getY() > m_yMax)) {
  ...
  /**
   * Returns the pose of the current state so position can be checked before the output is updated.
   *
   * @return The current state pose.
   */
  public Pose2d getStatePose() {
    return new Pose2d(
        getState(State.kX), getState(State.kY), new Rotation2d(getState(State.kHeading)));
  }
